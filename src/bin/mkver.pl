...
&fMsg(5, "gpDef=$gpDef cgBaseName=$cgBaseName", __FILE__, __LINE__);
if ("$gpExt" eq "") {
        $gpExt = "h pl php epm mak env java cs xml";
}
@tExt = split / /, $gpExt;
$gpExt = " " . $gpExt . " ";
$tDefault = " epm env ";
...
# ----
$phpFile = &fDefault("phpFile", "$phpFile", "$cgBaseName.php");
$tVar{"phpFile"} = "$phpFile";

$phpHeader = &fDefault("phpHeader", "$phpHeader", "$cgPhpHeader");
$phpFooter = &fDefault("phpFooter", "$phpFooter", "$cgPhpFooter");
...
# --------------
foreach $i ("cs", "env", "h", "java", "mak", "php", "pl", "xml") {
